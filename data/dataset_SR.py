...
class DatasetSR(data.Dataset):
    def __init__(self, opt):
        ...
        if self.phase == 'train':
            if 'nyu' in opt['dataroot_H'][0]:
                print(opt['dataroot_H'])
        ...
        self.paths_L = get_image_paths(opt['dataroot_L'], self.get_image)
        self.paths_H = get_image_paths(opt['dataroot_H'], self.get_image)

        self.tasks = len(opt['dataroot_H'])
        assert self.paths_H, 'Error: H path is empty.'
        print('All HR|LR images {}|{}'.format(len(self.paths_H), len(self.paths_L)))
